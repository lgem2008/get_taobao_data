...
tb_password = ''
#---------------------------------------------------------#
# get_data_start_num: 开始访问的订单编号
# get_data_total_num: 需要访问的订单数量
# num_each_page: 每页的订单数量
#---------------------------------------------------------#
get_data_start_num = 1
get_data_total_num = 300
num_each_page = 50
#---------------------------------------------------------#
# csv: csv导出的路径
#---------------------------------------------------------#
...
print("开始获取整个表格数据...") 
for page in range(ceil(get_data_total_num/num_each_page)):
    print("共需爬取"+str(ceil(get_data_total_num/num_each_page))+"页，正在爬取第"+str(page+1)+"页...") 
    tables = driver.find_elements(By.CLASS_NAME, 'js-order-container')
    for num in range(num_each_page if page+1 < get_data_total_num/num_each_page else get_data_total_num-page*num_each_page):
        try:
            print("开始获取第"+str(page*num_each_page+num+1)+"个商家的全部信息...")       
            # table_xpath = 'div[' + str(num) + ']'
            # table1 = dataView_info.find_element(By.XPATH, table_xpath)   #第一个商家的物品
            table1 = tables[num]

            print("开始获取第"+str(page*num_each_page+num+1)+"个商家的全部信息的表头...")
            ...
            print("开始获取第"+str(page*num_each_page+num+1)+"个商家的全部信息的商品数据...")
            ...
            for i in range(before_add_numbers):
                ...
                print("购买第"+str(page*num_each_page+num+1)+"个商家("+goods_business_name.text+")的第"+str(i+1)+"商品为: "+goods_name.text)
                ...
                if(i==0):   #商家名称,订单号,总价,购买时间只需要记录一次
                    ...
                    with open(csv_save_path, 'a', newline='', encoding='utf-8-sig') as file:
                        writer = csv.writer(file)
                        # writer.writerow([goods_business_name.text,'=HYPERLINK("'+goods_link+'", "'+goods_name.text+'"',goods_choice,goods_num.text,goods_price.text,goods_link,goods_order_id.text,goods_all_price.text,goods_data.text])
            ...
            print("=====================================================================================================")
            print("购买第"+str(page*num_each_page+num+1)+"个商家("+goods_business_name.text+")的商品数量为: "+str(before_add_numbers))
            print("购买第"+str(page*num_each_page+num+1)+"个商家("+goods_business_name.text+")的总金额为  : "+goods_all_price.text)
            print("=====================================================================================================")
