...
get_data_start_num = 9
get_data_total_num = 1
#---------------------------------------------------------
# csv: csv导出的路径
...
print("-----------------------1-----------------------")
table_heads = ['商家名称','商品名称','商品套餐','商品数量','商品价格','订单号','总价格','购买日期']
...
# //*[@id="tp-bought-root"]/div[4]/div/table/tbody[2]/tr[2]/td[1]/div/div[2]/p[1]/a[1]/span[2]      第二个商品名称
        for i in range(before_add_numbers):
            ...
            if(i==0):   #商家名称,订单号,总价,购买时间只需要记录一次
                ...
                with open(csv_save_path, 'a', newline='', encoding='utf-8-sig') as file:
                    writer = csv.writer(file)
                    writer.writerow([goods_business_name.text,goods_name.text,goods_choice,goods_num.text,goods_price.text,goods_order_id.text,goods_all_price.text,goods_data.text])
            else: 
                with open(csv_save_path, 'a', newline='', encoding='utf-8-sig') as file:
                    writer = csv.writer(file)
